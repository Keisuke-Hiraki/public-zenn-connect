...
通常の Lambda では途中で状態を保持できませんが、Durable Functions はチェックポイント＆リプレイの仕組みにより、関数が再起動されても前回の続きから処理を再開します。`context.wait()` でサスペンドしている間はコンピューティングリソースが消費されないため、数時間・数日にわたる待機も追加のコンピューティングコストなく実現できます。
...
![](/images/ChatGPT_Image_2026年6月23日_20_52_15.png)
