...
AI エージェントを使ってコードのセキュリティレビューをしたことはありますか？「このコードのセキュリティをチェックして」と頼むと、それなりの結果は返ってきます。しかし、その結果は本当に信頼できるものでしょうか。
...
:::message alert
WebGoat は教育目的で意図的に脆弱性を含んだアプリケーションです。実際のプロダクションコードではありません。
:::

「このコードのセキュリティ問題をレビューして」と AI に依頼したところ、以下のような結果が返ってきました。

:::details 素の AI によるレビュー結果（全文）
...
重要度別に整理されており、主要な脆弱性を捉えている点は評価できます。しかしこの結果には、いくつか気になる点があります。

- **「意図された脆弱性」と「意図されていない問題」の区別がない**: WebGoat は教育目的のアプリなので、学習用に意図して組み込まれた脆弱性と、設計上の本当の問題は分けて考える必要があります
- **複数の問題が組み合わさって成立する攻撃経路（リスクチェーン）が見えない**: 個別の問題はリストアップされていますが、「この問題とあの問題が組み合わさると RCE になる」という連鎖的な視点がありません
- **根拠となる標準の参照がない**: 「なぜこれが問題か」を OWASP 等の標準に紐づけた説明がなく、レポートの監査可能性が低い

これらは AI の能力の問題ではなく、**何の手順に従うかが指定されていない**ことが原因です。

## OWASP Secure Agent Playbook Project とは

https://github.com/OWASP/secure-agent-playbook

OWASP Secure Agent Playbook は、AI エージェントがセキュリティエンジニアリングタスクを実行するための、OWASP 標準に基づいた構造化された手順書（プレイブック）集です。

コードにインポートするフレームワークやライブラリではありません。各「プレイ」は SOC アナリストのプレイブックを AI エージェント向けに書き直したもので、エージェントが従うべき手順書です。
...
インストール後は、Claude Code の会話の中でスラッシュコマンドとしてスキルが利用できるようになります。

## OWASP Secure Agent Playbook を使うと何が変わるか

では、同じ WebGoat に対して、OWASP Secure Agent Playbook の `/code-review-security` スキルを使ってレビューしてみます。

:::message
環境の関係上、プレイブックなしは Claude Code 以外の AI エージェントを使用しましたが、モデルのバージョンは比較用に揃えています。
:::

:::details OWASP Secure Agent Playbook によるレビュー結果（全文）
...
| ファイル・行番号の特定 | 曖昧 | `UserService.java:53` のように正確 |
| 修正コードの提示 | 指針のみ | 具体的なコードスニペット付き |

プレイブックを使うと、AI モデルの能力ではなく**レビュー工程が構造化・標準化・監査可能な状態**になっていることがわかります。これがまさに OWASP Secure Agent Playbook の提供する価値です。

### プレイブックはどのような思考ロジックで動いているか

なぜこれだけ差が出るのか、`code-review-security` プレイの内部構造を F-07（SQL スキーマ注入）を例に解説します。
...
プレイはいきなり「危ないコードを探す」から始まりません。最初のステップでレビュー対象の文脈を 4 軸で確定させます。

- Language/Framework: どの脆弱性クラスが関連するかを決める
- Trust boundary: サーバーサイド / クライアントサイド / ライブラリ / インフラコードの区別
- Data sensitivity: PII・認証情報・金融データを扱うか
- Exposure: インターネット公開 / 社内のみ / ローカルのみ
...
1. Injection（A03 / CWE-74）
2. Authentication & Session（A07 / CWE-287）
3. Authorization（A01 / CWE-862）
4. Cryptography（A02 / CWE-327）
5. Data Exposure（A01 / CWE-200）
6. Security Misconfiguration（A05）
7. Deserialization & Data Integrity（A08 / CWE-502）

素の AI に「レビューして」と頼んだだけでは、この審査順序は存在しません。見つかった順・重大そうに見えた順でリストアップされます。プレイでは Injection を最初に審査するよう定められているため、`UserService.java:53` の SQL スキーマ注入が最優先で掘り下げられます。さらに各クラスの中に「SQL Injection の場合はクエリ内の文字列連結・パラメータ化の欠如・ORM の raw クエリを見よ」のように具体的なチェック項目が列挙されており、見落としを構造的に防ぎます。各チェック項目には OpenCRE のリンクが直接埋め込まれており（例: Injection には `[OpenCRE 161-451](https://www.opencre.org/cre/161-451)`）、ASVS・WSTG・NIST 800-53 への横断参照が自動解決されます。

**Step 3: フレームワーク固有のチェックを追加する**

Step 1 で確定した文脈をもとに、フレームワーク固有のリスクをさらに追加します。WebGoat は Spring を使っているため「SpEL インジェクション・Actuator の公開・マスアサインメント」という Spring 固有のチェックリストが自動的に追加されます。F-03（Actuator エンドポイントの認証なし公開）はこのステップで検出されます。素の AI はフレームワークの特性を文脈から推測しますが、チェック項目の列挙漏れが起きやすいです。

**Step 4: PR レビューの場合は差分に特化した分析をする**

今回はコードベース全体を対象にしているため Step 4 は適用外ですが、PR レビュー時はこのステップが機能します。変更行とその周辺の未変更コードに既存の security control が維持されているかを確認し、新しいエンドポイントに既存の認証パターンと整合した認可が付与されているかを検証します。削除された validation や auth check も検出対象になります。

**Step 5: 攻撃シナリオを具体的に記録する**

各ファインディングは「ファイル:行番号・脆弱なコードスニペット・攻撃シナリオの具体的説明・修正済みコード・確信度（HIGH / MEDIUM / LOW）」の形式で記録するよう義務付けられています。F-07 に対しては攻撃シナリオとして「`UserForm.java` のバリデーションで大半はブロックされるが、OAuth 登録パス経由でバイパスされる可能性がある」と記述されます。「どこから到達できるか」を明示することで、単なる脆弱性列挙を超えた記録になります。

**リスクチェーンは `security-team-lead` が担う**

ここまでが `code-review-security` プレイ単体の役割です。比較結果に出てきた「チェーン A〜C」は、`security-team-lead` エージェントが複数のスペシャリストの結果を統合する際に生成されます。`code-security-reviewer`（コードの問題）と `dependency-auditor`（依存関係の CVE）の結果が揃ってはじめて「F-06 の `commons-collections` と F-07 の SQL インジェクションを組み合わせると RCE になる」という連鎖が検出されます。一つのプレイではなく、エージェントのチームワークで成立している点が重要です。

これら一連のロジックは、熟練したセキュリティエンジニアが頭の中で自然にやっていることをプレイブックとして明文化したものです。AI が従う手順書として渡すことで、モデルの能力差によらず一定品質のレビューを再現できます。

## おまけで

ここに最新の脆弱性情報をコンテキストとして付与してあげるともっと効果が上がるのではないか？と思い、上位スキルとして以下を作成してみました。

:::details secure-agent-context-enricher スキル定義（全文）

```markdown
---
name: secure-agent-context-enricher
description: Auxiliary vulnerability context provider for OWASP Secure Agent Playbook. Extracts CVE candidates from project dependencies, enriches them with CISA KEV, FIRST EPSS, NVD CVSS v4.0/v3.1, CVE Record/CNA, and CISA Vulnrichment, then passes the enriched context to OWASP Secure Agent Playbook as supporting material for security code review. OWASP Secure Agent Playbook remains the primary review engine and finding authority.
license: CC-BY-4.0
---
...
- Do NOT create `vulnerability_context.json` or any other file
...
ベースの OWASP Secure Agent Playbook に対して、このスキルは 4 つのフェーズを前段・後段に追加しています。
...
ベースのプレイブックは CVE 番号と CVSS スコアは参照しますが、「その CVE が今現在実際に悪用されているか（KEV）」「悪用される確率（EPSS）」という**脅威インテリジェンス**は持っていません。ここが最大の差分です。
...
つまりこのスキルは「OWASP Secure Agent Playbook のレビュー品質はそのままに、CVE の優先度判断に脅威インテリジェンスを加える」という役割分担になっています。

### 結果について

結果は以下のように返ってきました。

:::details 脅威インテリジェンス付きレビュー結果（全文）
...
| ID       | 題目                                                                         | CVE                                                 |
| -------- | ---------------------------------------------------------------------------- | --------------------------------------------------- |
| **F-01** | XStream 1.4.5 RCE デシリアライゼーション — **KEV 登録済み (EPSS 0.94)**      | CVE-2021-39144                                      |
...
**最優先問題が変わった**。素の AI もプレイブックも `NoOpPasswordEncoder`（平文パスワード保存）を Critical の筆頭に挙げていました。しかし脅威インテリジェンスを加えると、XStream 1.4.5 の RCE（CVE-2021-39144）が筆頭になります。CISA KEV への登録（実際の野外悪用確認）と EPSS 0.94（今後 30 日以内の悪用確率が上位 6%）という情報が加わることで、「技術的に深刻」ではなく「今すぐ実際に攻撃に使われる可能性が高い」という軸で優先度が再評価されます。

**チェーン D が新たに検出された**。XStream の SSRF 脆弱性（CVE-2020-26258）が AWS IMDS（インスタンスメタデータサービス）への到達経路に繋がることが示されました。これはコードの静的解析だけでは見えにくい問題で、「この CVE を持つコンポーネントがクラウド環境のトラストバウンダリにどう影響するか」という脅威インテリジェンス側の文脈があって初めて検出できます。
