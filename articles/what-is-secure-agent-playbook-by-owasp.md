...
**最優先問題が変わった**  
素の AI もプレイブックも `NoOpPasswordEncoder`（平文パスワード保存）を Critical の筆頭に挙げていました。しかし脅威インテリジェンスを加えると、XStream 1.4.5 の RCE（CVE-2021-39144）が筆頭になります。CISA KEV への登録（実際の野外悪用確認）と EPSS 0.94（今後 30 日以内の悪用確率が上位 6%）という情報が加わることで、「技術的に深刻」ではなく「今すぐ実際に攻撃に使われる可能性が高い」という軸で優先度が再評価されます。

**チェーン D が新たに検出された**  
XStream の SSRF 脆弱性（CVE-2020-26258）が AWS IMDS（インスタンスメタデータサービス）への到達経路に繋がることが示されました。これはコードの静的解析だけでは見えにくい問題で、「この CVE を持つコンポーネントがクラウド環境のトラストバウンダリにどう影響するか」という脅威インテリジェンス側の文脈があって初めて検出できます。
