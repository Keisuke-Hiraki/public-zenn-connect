...
```mermaid
flowchart TD
    A[EC2 インスタンス] --> B{アカウントの<br/>スキャンモードは？}
    B -->|エージェントベーススキャン| C{SSM Agent は<br/>動作している？}
    B -->|ハイブリッドスキャン| D{SSM Agent は<br/>動作している？}

    C -->|いいえ| E[スキャン対象外<br/>UNMANAGED]
    C -->|はい| F

    D -->|いいえ| G[エージェントレススキャン<br/>EC2_AGENTLESS]
    D -->|はい| F{VM Scanner は<br/>インストールされている？}

    F -->|自動 / 手動インストール済み| H[エージェントベーススキャン<br/>EC2_INSPECTOR_AGENT_BASED]
    F -->|未インストール| I{SSM 経由で<br/>自動配布される？}

    I -->|SSM マネージド登録済み| H
    I -->|SSM マネージド未登録| J[スキャン対象外<br/>手動インストールするまで待機]

    J -.->|VM Scanner を手動インストール| H
```

SSM マネージド登録の有無は VM Scanner が自動配布されるかどうかを左右します。しかし、最終的にエージェントベーススキャンとして認識されるかどうかは「VM Scanner がインストールされ、実行されて Inspector にテレメトリを送信できたか」で決まります。SSM に依存しないルートが用意されている点が、今回の検証で見えてきた VM Scanner の特徴です。
