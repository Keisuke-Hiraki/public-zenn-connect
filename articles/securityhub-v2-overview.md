...
- **Security Finding（class_uid: 2001）** — 体制管理・コンプライアンス検出結果（CSPM）
- **Detection Finding（class_uid: 2004）** — 脅威検出（GuardDuty 等）
...
```json
{
  ...
  "metadata": {            // 検出結果のメタ情報
    "uid": "arn:aws:securityhub:ap-northeast-1:111122223333:finding/abc123",
    "product": {
      "name": "Security Hub",
      "vendor_name": "Amazon Web Services",
      "uid": "arn:aws:securityhub:ap-northeast-1::product/aws/securityhub",
      "feature": {
        "uid": "aws-foundational-security-best-practices/v/1.0.0/S3.1"
      }
    },
    "version": "1.6"
  },
  ...
  "finding_info": {        // 検出結果の詳細（ASFF の Title/Description に相当）
    ...
    "desc": "This control checks if S3 buckets have bucket-level public access blocks applied.",
    ...
    "src_url": "https://docs.aws.amazon.com/securityhub/...",
  ...
  "compliance": {          // コンプライアンス情報
    "control": "S3.1",
    "standards": ["AWS Foundational Security Best Practices v1.0.0"],
    "status": "FAILED",
    "status_id": 1,
    "assessments": [
      {
  ...
  "remediation": {         // 修復ガイダンス
    "desc": "Enable S3 Block Public Access settings at the bucket level.",
    "references": [
  ...
  "confidence_score": 95,
  "comment": ""            // Automation Rules / BatchUpdateFindingsV2 で更新可能
}
```
...
| **主な OCSF クラス** | Security Finding (2001), Detection Finding (2004), Vulnerability Finding (2002) | API Activity, Network Activity, DNS Activity, Authentication, Security Finding | 変換元ログ種別に依存 |
