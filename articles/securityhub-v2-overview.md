...
| 対象 | 料金 |
|---|---|
| CloudTrail 管理イベント | $4.72 / 100万イベント |
| データイベント、ネットワークアクティビティ、その他のログ（最初の 1,000 GB） | $0.649 / GB |
| データイベント、ネットワークアクティビティ、その他のログ（〜10,000 GB） | $0295 / GB |
| データイベント、ネットワークアクティビティ、その他のログ（10,000 GB 超） | $0.118 / GB |

データイベント、ネットワークアクティビティ、その他のログには、Amazon VPC フローログ、Amazon Route 53 DNS クエリログ、Amazon S3 データイベント (100 万イベント = 2 GB)、Amazon EKS 監査ログ (100 万イベント = 2.2 GB)、および AWS Lambda ネットワークログが含まれます。

### Lambda コードスキャン アドオン

Inspector を使用した AWS Lambda コードスキャンの料金：

| 対象 | 料金 |
|---|---|
| AWS Lambda コードスキャン | $0.495 / 関数あたり |
